...
public class CraftAction implements PlayerActionHandler {
	...
	public CraftAction (ConfigProvider configProvider) {
	...
	public CraftAction (int productID, int animation, int craftTime, String successText) {
		ObjType productType = ObjTypeList.getInstance().list(productID);
		EnumType levelEnum = configProvider.getEnumTypes().list(681);
	...
	private void craftSuccess (Player player) {
		...
		if (skill != null) {
			...
			player.getVars().setVarValueInt(1176, xp);//Increment xp received value in the crafting process interface
		}
	...
	private void removeStructMaterials (Player player, int structID, int... slots) {
		StructType struct = configProvider.getStructTypes().list(structID);
